...
public class FindExpCar {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        return cars.max(Comparator.comparingInt(Car::getPrice)).stream().findFirst();
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        return cars.filter((o1)->o1.getPrice()> mostExpensiveCar.getPrice()).findFirst();
    }
}
